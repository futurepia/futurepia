...
class debug_private_key_storage : public private_key_storage
{
   public:
      ...
      std::map< futurepia::chain::public_key_type, fc::ecc::private_key > key_table;
};

class debug_node_api_impl
{
   public:
      debug_node_api_impl( futurepia::app::application& _app );
      ...
      //void debug_push_block( const futurepia::chain::signed_block& block );
      ...
      std::shared_ptr< futurepia::plugin::debug_node::debug_node_plugin > get_plugin();
...
)
{
   ...
   if( (pubkey == futurepia::chain::public_key_type()) || (gen_pub == pubkey) )
   {
      result = gen_priv;
      return;
   }
...
debug_node_api_impl::debug_node_api_impl( futurepia::app::application& _app ) : app( _app )
{
...
void debug_node_api_impl::debug_mine( debug_mine_result& result, const debug_mine_args& args )
{
   ...
   if( args.props.valid() )
      ...
      op.props = db->get_bobserver_schedule_object().median_props;
   ...
   tx.set_expiration( db->head_block_time() + FUTUREPIA_MAX_TIME_UNTIL_EXPIRATION );

   tx.sign( *priv, FUTUREPIA_CHAIN_ID );
...
uint32_t debug_node_api_impl::debug_push_blocks( const std::string& src_filename, uint32_t count, bool skip_validate_invariants )
{
   ...
   std::shared_ptr< futurepia::chain::database > db = app.chain_database();
     ...
     )
   {
      ...
      futurepia::chain::block_log log;
      log.open( src_path );
      uint32_t first_block = db->head_block_num()+1;
      uint32_t skip_flags = futurepia::chain::database::skip_nothing;
      if( skip_validate_invariants )
         skip_flags = skip_flags | futurepia::chain::database::skip_validate_invariants;
      for( uint32_t i=0; i<count; i++ )
      {
         //fc::optional< futurepia::chain::signed_block > block = log.read_block( log.get_block_pos( first_block + i ) );
         uint64_t block_pos = log.get_block_pos( first_block + i );
         if( block_pos == futurepia::chain::block_log::npos )
         {
...
{
   return get_plugin()->debug_generate_blocks( debug_key, count, futurepia::chain::database::skip_nothing, 0, &key_storage );
}
...
{
   return get_plugin()->debug_generate_blocks_until( debug_key, head_block_time, generate_sparsely, futurepia::chain::database::skip_nothing, &key_storage );
}

fc::optional< futurepia::chain::signed_block > debug_node_api_impl::debug_pop_block()
{
   std::shared_ptr< futurepia::chain::database > db = app.chain_database();
   return db->fetch_block_by_number( db->head_block_num() );
}

/*void debug_node_api_impl::debug_push_block( const futurepia::chain::signed_block& block )
{
   app.chain_database()->push_block( block );
}*/

futurepia::chain::bobserver_schedule_object debug_node_api_impl::debug_get_bobserver_schedule()
{
   return app.chain_database()->get( futurepia::chain::bobserver_schedule_id_type() );
}

futurepia::chain::hardfork_property_object debug_node_api_impl::debug_get_hardfork_property_object()
{
   return app.chain_database()->get( futurepia::chain::hardfork_property_id_type() );
}
...
std::shared_ptr< futurepia::plugin::debug_node::debug_node_plugin > debug_node_api_impl::get_plugin()
{
...
{
   return app.chain_database()->get( futurepia::chain::hardfork_property_id_type() ).last_hardfork >= hardfork_id;
}
...
debug_node_api::debug_node_api( const futurepia::app::api_context& ctx )
{
...
fc::optional< futurepia::chain::signed_block > debug_node_api::debug_pop_block()
{
   return my->debug_pop_block();
}

/*void debug_node_api::debug_push_block( futurepia::chain::signed_block& block )
{
   my->debug_push_block( block );
}*/

futurepia::chain::bobserver_schedule_object debug_node_api::debug_get_bobserver_schedule()
{
   return my->debug_get_bobserver_schedule();
}

futurepia::chain::hardfork_property_object debug_node_api::debug_get_hardfork_property_object()
{

#include <futurepia/market_history/market_history_api.hpp>
...
{
   public:
      market_history_api_impl( futurepia::app::application& _app )
         :app( _app ) {}
...
market_ticker market_history_api_impl::get_ticker() const
{
   ...
   {
      auto open = ( asset( itr->open_fpch, FPCH_SYMBOL ) / asset( itr->open_futurepia, FUTUREPIA_SYMBOL ) ).to_real();
      result.latest = ( asset( itr->close_fpch, FPCH_SYMBOL ) / asset( itr->close_futurepia, FUTUREPIA_SYMBOL ) ).to_real();
   ...
   result.futurepia_volume = volume.futurepia_volume;
   result.fpch_volume = volume.fpch_volume;
...
market_volume market_history_api_impl::get_volume() const
{
   ...
   do
   {
      result.futurepia_volume.amount += itr->futurepia_volume;
      result.fpch_volume.amount += itr->fpch_volume;
...
order_book market_history_api_impl::get_order_book( uint32_t limit ) const
{
   ...
   const auto& order_idx = app.chain_database()->get_index< futurepia::chain::limit_order_index >().indices().get< futurepia::chain::by_price >();
   auto itr = order_idx.lower_bound( price::max( FPCH_SYMBOL, FUTUREPIA_SYMBOL ) );

   order_book result;

   while( itr != order_idx.end() && itr->sell_price.base.symbol == FPCH_SYMBOL && result.bids.size() < limit )
   {
      order cur;
      cur.price = itr->sell_price.base.to_real() / itr->sell_price.quote.to_real();
      cur.futurepia = ( asset( itr->for_sale, FPCH_SYMBOL ) * itr->sell_price ).amount;
      cur.fpch = itr->for_sale;
      result.bids.push_back( cur );
      ++itr;
   }

   itr = order_idx.lower_bound( price::max( FUTUREPIA_SYMBOL, FPCH_SYMBOL ) );

   while( itr != order_idx.end() && itr->sell_price.base.symbol == FUTUREPIA_SYMBOL && result.asks.size() < limit )
   {
      order cur;
      cur.price = itr->sell_price.quote.to_real() / itr->sell_price.base.to_real();
      cur.futurepia = itr->for_sale;
      cur.fpch = ( asset( itr->for_sale, FUTUREPIA_SYMBOL ) * itr->sell_price ).amount;
...
market_history_api::market_history_api( const futurepia::app::api_context& ctx )
{

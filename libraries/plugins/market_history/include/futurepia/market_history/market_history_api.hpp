...
#include <futurepia/market_history/market_history_plugin.hpp>
...
struct market_ticker
{
   ...
   asset       futurepia_volume = asset( 0 , FUTUREPIA_SYMBOL );
   asset       fpch_volume = asset( 0, FPCH_SYMBOL );
};
...
{
   public:
      market_history_api( const futurepia::app::api_context& ctx );

      void on_api_startup();

      /**
       * @brief Returns the market ticker for the internal FPCH:FPC market
       */
      ...
      /**
       * @brief Returns the current order book for the internal FPCH:FPC market.
      ...
      /**
       * @brief Returns the trade history for the internal FPCH:FPC market.
      ...
      /**
       * @brief Returns the N most recent trades for the internal FPCH:FPC market.
      ...
      /**
       * @brief Returns the market history for the internal FPCH:FPC market.
...
FC_REFLECT( futurepia::market_history::market_ticker,
   (latest)(lowest_ask)(highest_bid)(percent_change)(futurepia_volume)(fpch_volume) );
FC_REFLECT( futurepia::market_history::market_volume,
   (futurepia_volume)(fpch_volume) );
FC_REFLECT( futurepia::market_history::order,
   (price)(futurepia)(fpch) );
FC_REFLECT( futurepia::market_history::order_book,
   (bids)(asks) );
FC_REFLECT( futurepia::market_history::market_trade,
   (date)(current_pays)(open_pays) );

FC_API( futurepia::market_history::market_history_api,
   (get_ticker)
   (get_volume)
   (get_order_book)
   (get_trade_history)

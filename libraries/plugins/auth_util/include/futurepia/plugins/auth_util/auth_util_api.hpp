...
{
   public:
      auth_util_api( const futurepia::app::api_context& ctx );
...
FC_REFLECT( futurepia::plugin::auth_util::check_authority_signature_params,
   (account_name)
   (level)
   (dig)
   (sigs)
   )
FC_REFLECT( futurepia::plugin::auth_util::check_authority_signature_result,
   (keys)
   )

FC_API( futurepia::plugin::auth_util::auth_util_api,

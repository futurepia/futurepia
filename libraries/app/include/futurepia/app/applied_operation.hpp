...
{
   applied_operation();
   applied_operation( const futurepia::chain::operation_object& op_obj );

   futurepia::protocol::transaction_id_type trx_id;
   uint32_t                               block = 0;
...
FC_REFLECT( futurepia::app::applied_operation,
   (trx_id)
   (block)
   (trx_in_block)
   (op_in_trx)

...
#if defined(WIN32)
         wchar_t* wcExceptionMessage = const_cast<wchar_t*>(dir.native().c_str());
         std::wstring wsExceptionMessage(wcExceptionMessage);
         std::string strExceptionMessage(wsExceptionMessage.begin(), wsExceptionMessage.end());

         if( !write ) BOOST_THROW_EXCEPTION(std::runtime_error("database file not found at " + strExceptionMessage));
#else
